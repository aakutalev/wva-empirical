...
class Model:
    def __init__(self, shape, session, learning_rate=0.01, impact_to_grads=False):
        ...
        if impact_to_grads:
            # self._train_step = _WVA_SGD(learning_rate).minimize(self.loss, impacts=impacts)
...
exp_file = 'wva-adam-1x-grads.dmp'
try:
...
model = Model(net_struct, sess, learning_rate, True)

start_time = datetime.datetime.now()
time_format = "%Y-%m-%d %H:%M:%S"
print(f'Continual learning start at {start_time:{time_format}}')

for lmbda in np.arange(50., 4100., 200.):
